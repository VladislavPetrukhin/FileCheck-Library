...
namespace FileCheck {
    ...
    private void test_print_left_context() {
        ...
        string result = get_left_context(buffer, 3, 0);
    ...
    private void test_print_right_context(){
        uint8[] buffer = {0x01, 0x02, 0x03, 0x04, 0x05};
        string result = get_right_context(buffer, 1, 5);
    ...
    private void test_compare_files() {
        ...
        compare_files.begin(original_file, corrupted_file, (obj, res) => {
            try {
                var results = compare_files.end(res);
    ...
    // Recursive function for traversing all files in a directory
                ...
                if (original_info.get_file_type() == FileType.DIRECTORY) {
                    ...
                    compare_files.begin(original_file,corrupted_file, (obj, res) => {
                        try {
                            ...
                            results.add_all(partial_results); // add results to ArrayList
                        } catch (ThreadError e) {
                            ...
                            if(!is_last_corrupted){
                                ...
                                if (results.size > 0) {
                                    ...
                                    if (last_result.contains(" byte:") && last_result.contains("orig=") && last_result.contains("corr=")) {
                                        ...
                                        result += "right: " + get_right_context(corrupted_buffer, offset + i,corrupted_read);
